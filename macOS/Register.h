...
public:
    ...
    //处理客户端/服务器请求，根据请求类型调用相应函数，得到结果并返回给客户端/服务器
...
void Register::readyToServe(bool ipv4,bool ipv6){
    ...
    if(ipv4){
        ...
        if (::bind(serverSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == -1) {
            cout << "绑定失败，请检查ipv4地址与端口号是否正确" << endl;
            close(serverSocket);
            return;
        }
    ...
    if(ipv6){
        ...
        inet_pton(AF_INET6, listenipnum6.c_str(), &serverAddr6.sin6_addr);
        if (::bind(serverSocket6, (struct sockaddr*)&serverAddr6, sizeof(serverAddr6)) == -1) {
            cout << "绑定失败，请检查ipv6地址与端口号是否正确" << endl;
            close(serverSocket6);
            return;
        }
...
void Register::handleclient(int clientSocket){
    string info;
    ssize_t bytesReceived = recvWithTimeout(clientSocket, info, 1024*1024*10);
    if (bytesReceived == -1) {
        cout << "接收数据失败" << endl;
        close(clientSocket);
        return;
    ...
    //cout<<reply<<endl;
    // 发送回复给客户端
    ssize_t bytesSent = sendAll(clientSocket, reply.c_str(), reply.size());
    if (bytesSent == -1) {
        cout << "发送回复失败" << endl;
    }
    // 关闭客户端套接字
...
string Register::addServer(string serverinfo){
    ...
    //返回注册成功信息，并返回当前服务器ID
    //解锁
    return "OK "+to_string(id)+" "+getLocalIPAddress()+" "+portnum;//把注册中心的IPv4地址和端口号返回给服务器，用于心跳
}
...
string Register::showtoClient(string functionname){
    ...
    for(int i=0;i<serverList.size();i++){
        ...
        reply += "该服务器的IPv4："+serverList[i].selfipnum + "\n";
        reply += "该服务器的IPv6："+serverList[i].selfipnum6 + "\n";
...
void Register::hearingheartbeat(){
    ...
    while (true) {
        ...
        socklen_t clientAddrSize = sizeof(clientAddr);
        char buffer[1024] = {0};
        ssize_t bytesReceived = recvfrom(sock, buffer, 1024, 0, (struct sockaddr*)&clientAddr, &clientAddrSize);
        // 下面代码跑的挺快的，不需要多线程
        if (bytesReceived == -1) {
            cout << "接收数据失败" << endl;
            continue;
        ...
        ssize_t bytesSent = sendto(sock, reply.c_str(), reply.size(), 0, (struct sockaddr*)&clientAddr, clientAddrSize);
        if (bytesSent == -1) {
            cout << "发送回复失败" << endl;
        }
    }
