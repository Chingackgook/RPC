...
int main(int argc,char* argv[]){
    ...
    string listenip="0.0.0.0",listenip6="::";
    string tempinip="",porttemp="-1";
    string port4="22222",port6="22223";
    if(argc==1){//默认启动，仅供测试
        goto defaultstart;
    }
    ...
    time_t seconds;
    seconds = time(NULL);
    while(!isgoodport(port4)&&ip4ok){
        cout<<"ipv4端口号不合法，请重新输入"<<endl;
        cin>>port4;
    }
    while(!isgoodport(port6)&&ip6ok){
    ...
    cout<<"服务器初始化成功！"<<endl;
    server.addfunction("add","两数相加",stringtovec("2*double"),"double",(u64)add);
    server.addfunction("sub","两数相减",stringtovec("2*float"),"double",(u64)sub);
    server.addfunction("matrix5_5","计算5*5的行列式值，先行后列，输入25个int",stringtovec("25*int"),"int",(u64)matrix5_5);
    ...
    server.addfunction("quadratic","解一元二次方程，三个float依次为a,b,c",stringtovec("3*float"),"double&string",(u64)quadratic);
    server.addfunction("designatedtime","指定时间处理，输入时间(int)",stringtovec("1*int"),"string",(u64)designatedtime);
    server.addfunction("teststring","测试处理带空格的字符串",stringtovec("1*int 1*int 2*string"),"string",(u64)testString);
    server.addfunction("sendfile","发送文件给服务器",stringtovec("1*file"),"string",(u64)senfile);
    cout<<"请指定注册中心的ip地址:"<<endl;
    string reg_ip="",reg_port="";
    cin>>reg_ip;
