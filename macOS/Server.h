...
//写readytoserve函数，使其有两个线程，一个是ipv4的套接字，一个是ipv6的套接字，分别监听
...
void Server::init(string acceptipnum, string acceptipnum6, string portnum ,string portnum6,string selfipnum, string selfipnum6,int create_time){
    ...
    info.selfipnum6 = acceptipnum6=="::"?selfipnum6:acceptipnum6;
...
void Server::regist(string reg_ipnum, string reg_portnum,int reg_iptype){
    ...
    if(reg_iptype==4){
        ...
        inet_pton(AF_INET6, reg_ipnum.c_str(), &serverAddr6.sin6_addr);
    ...
    // 等待数据中心回复
    string response;
    ssize_t bytesReceived = recvWithTimeout(clientSocket, response, 1024, 10);
    if (bytesReceived <0) {
...
void Server::readyToServe(bool ok4,bool ok6){
    ...
    if(ok6){
        ...
        inet_pton(AF_INET6, info.acceptipnum6.c_str(), &serverAddr6.sin6_addr);
...
void Server::handleclient(int clientSocket) {
    ...
    //解锁
    string info;
    ssize_t bytesReceived = recvWithTimeout(clientSocket, info, -1);//-1表示无限制接收长度
    if(bytesReceived <=0){
        cout << "接收数据失败或超时" << endl;
        close(clientSocket);
        return;
...
void Server::heartbeats(){
    ...
    while(1){
        ...
        ssize_t bytesSent = sendto(clientSocket, hbinfo.c_str(), hbinfo.size(), 0, (struct sockaddr*)&serverAddr, sizeof(serverAddr));
        if (bytesSent == -1) {
            cout << "发送数据失败" << endl;
            close(clientSocket);
            return;
        ...
        char buffer[1024] = {0};
        ssize_t bytesReceived = recvfrom(clientSocket, buffer, 1024, 0, nullptr, nullptr);
...
void Server::beavailableornot(string functionname,bool sign){
    ...
    for(int i=0;i<info.functionInfo.size();i++){
        if(info.functionInfo[i].functionname==functionname){
            ...
            // 等待数据中心回复
            string buffer;
            ssize_t bytesReceived = recvWithTimeout(clientSocket, buffer, 1024, 10);
            if (bytesReceived == -1) {
