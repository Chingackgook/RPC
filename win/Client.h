...
bool Client::init(string reg_ipnum,string reg_portnum, int reg_iptype=4){
    ...
    if(reg_iptype==4){
        ...
        int len=sizeof(serv_addr6);
        WSAStringToAddressA((LPSTR)(reg_ipnum.c_str()), AF_INET6, NULL, (sockaddr*)&serv_addr6.sin6_addr, &len);
    ...
    if(reg_iptype==4){
        if(connectWithTimeout(sock,(struct sockaddr*)&serv_addr,sizeof(serv_addr),5)<0){
            cout<<"连接失败或超时，请检查注册中心ipv4地址和端口号是否正确"<<endl;
            return 0;
        }
    }
    else if(reg_iptype==6){
        if(connectWithTimeout(sock,(struct sockaddr*)&serv_addr6,sizeof(serv_addr6),5)<0){
            cout<<"连接失败或超时，请检查注册中心ipv6地址和端口号是否正确"<<endl;
            return 0;
        }
    }
...
handle Client::findServer(string functionname,string ipnum){
    ...
    if(reg_iptype==4){
        if(connectWithTimeout(sock,(struct sockaddr*)&serv_addr,sizeof(serv_addr),10)<0){   
            return handle("fail","连接失败或超时","no","failconnect",0);
        }
    }
    else if(reg_iptype==6){
        if(connectWithTimeout(sock,(struct sockaddr*)&serv_addr6,sizeof(serv_addr6),10)<0){
...
Rpc_data Client::callServer(handle hand,Rpc_data data){
    ...
    string info;
    int bytesReceived = (int)recvWithTimeout(sock, info, 1024*1024*10,40);
    if (bytesReceived == 0) {
